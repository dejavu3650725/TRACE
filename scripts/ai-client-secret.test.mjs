...
for (const file of sourceFiles) {
  ...
  assert.equal(/NEXT_PUBLIC_(?:GEMINI|AI_PROVIDER|AI_API_KEY)/.test(content), false, `${file} exposes AI configuration publicly`);
...
const configuredSecret = process.env.GEMINI_API_KEY;
if (configuredSecret) {
...
console.log(JSON.stringify({
  ...
  configuredSecretChecked: Boolean(configuredSecret),
}, null, 2));
